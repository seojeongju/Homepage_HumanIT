...
export async function onRequestGet(context) {
  ...
  try {
    ...
    // FAQ 목록 조회 (order_num으로 정렬)
    const result = await db.prepare(
      `SELECT id, question, answer, category, order_num, status, created_at, updated_at 
       FROM faqs 
       ${whereClause}
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    // FAQ 생성
    const result = await db.prepare(
      `INSERT INTO faqs (question, answer, category, order_num, status) 
       VALUES (?, ?, ?, ?, ?) 
